module hcsr04_sensor (
    ...
    localparam TRIG_PULSE = 1000;       // 10?s trigger pulse (100 MHz)
    localparam WAIT_TIME = 4000;        // 40?s wait before detecting echo
    ...
    always_ff @(posedge clk) begin
        case (state)
            // 1. Send 10?s Trigger Pulse (Active-Low)
            SEND_TRIGGER: begin
                trig <= 0;  // Active-low pulse start
                counter <= counter + 1;
                if (counter >= TRIG_PULSE) begin // 10?s pulse
                    trig <= 1;  // End of pulse
                    counter <= 0;
            ...
            // 2. Wait for 40?s before checking echo (to prevent false triggers)
